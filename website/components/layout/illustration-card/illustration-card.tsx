...
export default function IllustrationCard({ icon, className, style }: { icon: React.ReactNode, className?: string, style?: React.CSSProperties }) {
  return (
    <div
      className={cn(buttonUnidokaVariants.glass, `absolute flex items-center justify-center size-30 md:size-38 rounded-3xl transition-transform duration-1000 ease-out select-none pointer-events-none ${className}`)}
      style={style}
    >
