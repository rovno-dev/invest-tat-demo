...
import { ArrowRight, Cpu, Users, BuildingIcon, Rocket, TrendUp, Globe, WifiHighIcon, Code, GraduationCap, CheckCircle, Lightning } from "@phosphor-icons/react";
...
export default function InnopolisPage() {
  ...
  const t = zoneTranslations["innopolis"]?.[lang] ?? zoneTranslations["innopolis"]?.en ?? {};
        ...
        <Container>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div className="md:col-span-7 rounded-2xl bg-(--card) p-8 border border-(--outline)">
              ...
              <p className="text-sm text-(--on-bg-low) mt-1">{lang === "ru" ? "Компании" : "Companies"}</p>
            </div>
            <div className="md:col-span-4 rounded-2xl bg-(--card) p-8 border border-(--outline)">
              ...
              <p className="text-sm text-(--on-bg-low) mt-1">{lang === "ru" ? "Специалисты" : "Specialists"}</p>
            </div>
            <div className="md:col-span-8 rounded-2xl bg-(--card) p-8 border border-(--outline)">
              ...
              <p className="text-sm text-(--on-bg-low) mt-1">{lang === "ru" ? "Макс. грант для МСП" : "Max Grant for SMEs"}</p>
            </div>
          </div>
        </Container>
      </section>
        ...
        <Container>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div className="md:col-span-7 rounded-2xl bg-(--bg) p-8 border border-(--outline)">
              ...
              <p className="mt-3 text-body-3 text-(--on-bg-medium)">{t.feature1Description}</p>
            ...
            <div className="md:col-span-5 rounded-2xl bg-(--bg) p-8 border border-(--outline) flex flex-col justify-between">
              ...
              <div>
                ...
                <p className="mt-3 text-body-4 text-(--on-bg-medium)">{t.feature2Description}</p>
              </div>
            </div>
          </div>
        </Container>
              ...
              <Card key={item.title} className="group p-6 hover:border-(--primary)/50 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                <div className="flex items-center gap-4">
                  <div className="flex size-12 items-center justify-center rounded-xl text-(--primary)">
                    {index === 0 && <BuildingIcon className="size-6" weight="duotone" />}
                    {index === 1 && <WifiHighIcon className="size-6" weight="duotone" />}
                    {index === 2 && <Users className="size-6" weight="duotone" />}
                    {index === 3 && <Globe className="size-6" weight="duotone" />}
                    {index === 4 && <Lightning className="size-6" weight="duotone" />}
                    {index === 5 && <Cpu className="size-6" weight="duotone" />}
                  </div>
