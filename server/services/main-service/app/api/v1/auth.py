...
from app.shared.auth import create_access_token, create_refresh_token, decode_token, verify_password
...
async def phone_login_endpoint(
    payload: SmsLoginRequest,
    db: Session = Depends(get_db),
    _: None = Depends(limit_otp_send),
) -> PhoneSendCode:
    user_found = db.query(User).filter(User.phone == payload.phone).first()
